package com.kabouterlabs.ode.examples.ode1
...
object Ode1Example {
  def apply[A](constraints:Int=0)(implicit ev1: OdeSolverTC[A] {type SolverDataType = Double}): Unit = {
     ...
     */
    val range = for (index <- Range(0, 13)) yield  {
      if (index == 0) 0.0 else 0.04 * math.pow(10.0, index)
    }

    val eval = ivpsolver.solve(LineRange(range.toArray), Array(1.0, 0.0, 0.0))
    /*
...
object RunContext
{
  ...
  def sode(): Unit = {
    import com.kabouterlabs.ode.implicits.lsode.LsodeImplicit._
    Ode1Example()
  }
