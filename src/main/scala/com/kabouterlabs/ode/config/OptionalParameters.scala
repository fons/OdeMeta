...
object OptionalParameterType
{
  ...
  case object MIN_STEPS          extends OptionalParameterType
...
case class OptionalParameters(initialStepSize:Option[Double], minStepSize:Option[Double], maxStepSize:Option[Double],
                              minSteps:Option[Int],maxSteps:Option[Int],
                              minOrder:Option[Int], maxOrder:Option[Int],
                              maxOrderStiff:Option[Int], maxOrderNonStiff:Option[Int],
                              ...
                              rwork:Option[Array[Double]], iwork:Option[Array[Int]])
{
  ...
  override def toString: String = {
    val line = strd("initial step size", initialStepSize) +
      ...
      strd("maximum steps taken", minSteps) +
  ...
  def this(initialStep:Option[Double], minStepSize:Option[Double], maxStepSize:Option[Double], minSteps:Option[Int],
             maxSteps:Option[Int],minOrder:Option[Int], maxOrder:Option[Int], maxOrderStiff:Option[Int], maxOrderNonStiff:Option[Int],
           diagnostics:Option[Boolean]
            ) {
      this(initialStep,minStepSize,maxStepSize,minSteps,maxSteps,minOrder, maxOrder,maxOrderStiff,maxOrderNonStiff,diagnostics,None,None)
    }

    def this(r:Option[Array[Double]],l:Option[Array[Int]]) {
      this(None,None,None,None,None,None,None,None,None,None,r,l)
    }

    def ++(tp:OptionalParameterType,  vl:Double) = (tp, rwork) match {
      case (_, Some(_)) => this
      case (OptionalParameterType.INITIAL_STEP_SIZE,None) => new OptionalParameters(Some(vl), minStepSize, maxStepSize,minSteps,maxSteps,minOrder, maxOrder,maxOrderStiff,
        maxOrderNonStiff, diagnostics)
      case (OptionalParameterType.MIN_STEP_SIZE,    None) => new OptionalParameters(initialStepSize, Some(vl), maxStepSize,minSteps,maxSteps,minOrder, maxOrder,
        maxOrderStiff,maxOrderNonStiff, diagnostics)
      case (OptionalParameterType.MAX_STEP_SIZE,    None) => new OptionalParameters(initialStepSize, minStepSize, Some(vl),minSteps,maxSteps,minOrder, maxOrder,maxOrderStiff,
        maxOrderNonStiff, diagnostics)
      case (_,_) => this
    }
    ...
    def ++(tp:OptionalParameterType,  vl:Int) = (tp, iwork) match {
      ...
      case (OptionalParameterType.MIN_STEPS,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,Some(vl),maxSteps,minOrder, maxOrder,
        maxOrderStiff,maxOrderNonStiff, diagnostics)

      case (OptionalParameterType.MAX_STEPS,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps,Some(vl),minOrder, maxOrder,
        maxOrderStiff,maxOrderNonStiff, diagnostics)

      case (OptionalParameterType.MIN_ORDER,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps,maxSteps,Some(vl),maxOrder,
        maxOrderStiff,maxOrderNonStiff, diagnostics)

      case (OptionalParameterType.MAX_ORDER,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps, maxSteps,minOrder, Some(vl),
        maxOrderStiff,maxOrderNonStiff, diagnostics)

      case (OptionalParameterType.MAX_ORDER_STIFF,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps,maxSteps,minOrder, maxOrder,
        Some(vl),maxOrderNonStiff, diagnostics)

      case (OptionalParameterType.MAX_ORDER_NONSTIFF,None) => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps,maxSteps,minOrder, maxOrder,
        maxOrderStiff,Some(vl), diagnostics)

      case (_,_) => this
    }
  def ++ (tp:OptionalParameterType, vl:Boolean) = tp match {
    case OptionalParameterType.DIAGNOSTICS => new OptionalParameters(initialStepSize, minStepSize, maxStepSize,minSteps,maxSteps,minOrder,maxOrder,
