...
/**
  ...
  */
class Dop853 (dim:Int, funcM:OdeFuncM[Double], params:FuncParams[Double], config:Config)
{
    ...
    def set_itol(itol: Pointer[lang.Integer], rtol: Pointer[lang.Double], atol: Pointer[lang.Double]) = {
      (config.relativeTolerance.get, config.absoluteTolerance.get) match {
        case ((Some(rtolv), None), (Some(atolv), None)) => {
          itol.set(dop853_itol_e.ALL_SCALAR.value.toInt)
          rtol.set(rtolv)
          atol.set(atolv)
        }

        case ((None, Some(rtola)), (None, Some(atola))) => {
          itol.set(dop853_itol_e.ALL_ARRAY.value.toInt)
  ...
  private val func = new dop853_fcn_callback   {
  ...
  private val fcn: Pointer[dop853_fcn_callback] = Pointer.getPointer(func)
  ...
  private val iout:Pointer[lang.Integer]       = Pointer.pointerToInt(dop853_iout_e.NEVER_CALLED.value.toInt)
  ...
  def run(range:LineRangeT[Double], init:Array[Double]):Option[StackT] = HandleException {
    ...
    range.withRange((next:Double) => {
      ...
      dop853(n, fcn, x, y, xend,rtol,atol,itol,null,iout,work,lwork,iwork,liwork,null, null, idid)

      dop853_idid_e.fromValue(idid.get()) match {
        case c if c == dop853_idid_e.SUCCESS || c == dop853_idid_e.SUCCESS_INTR   => {
          stack.append(x.get())
          for (yval <- y.getDoubles(n.get())) stack.append(yval)
          Some(x.get())
        }

        case c if c == dop853_idid_e.INPUT_INCONSISTENT =>  {
          LogIt().error("input not cosistent + istate : " + c)
          None
        }

        case c if c == dop853_idid_e.NMAX_TOO_SMALL =>  {
          LogIt().error("larger nmax needed + istate : " + c)
          None
        }

        case c if c == dop853_idid_e.STEP_TOO_SMALL =>  {
          LogIt().error("step size too small + istate : " + c)
          None
        }

        case c if c == dop853_idid_e.STIFF_PROBLEM =>  {
