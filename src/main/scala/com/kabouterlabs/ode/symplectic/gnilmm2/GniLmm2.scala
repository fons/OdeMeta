package com.kabouterlabs.ode.symplectic.gnilmm2
...
case class GniLmm2(dim:Int, funcM:OdeFuncM[Double], params:FuncParams[Double], config:Config)
{
  private val logger = LogIt()


  private val method = config.method match {
    case Methods.SYMPLECTIC_2_STAGES => gni_lmm2_method_e.LMM2_METH_201.value.toInt
    case Methods.SYMPLECTIC_4_STAGES => gni_lmm2_method_e.LMM2_METH_401.value.toInt
    case Methods.SYMPLECTIC_801_STAGES => gni_lmm2_method_e.LMM2_METH_801.value.toInt
    case Methods.SYMPLECTIC_802_STAGES => gni_lmm2_method_e.LMM2_METH_802.value.toInt
    case Methods.SYMPLECTIC_803_STAGES => gni_lmm2_method_e.LMM2_METH_803.value.toInt
    case _             => gni_lmm2_method_e.LMM2_METH_201.value.toInt
  }

  private val func = new gni_lmm2_f_callback {
  ...
  private val func_sp: Pointer[gni_lmm2_f_callback] = Pointer.getPointer(func)
  ...
  def run(range: LineRangeT[Double], init: Array[Double]): Option[StackT] = HandleException {
    ...
    range.withRange((next:Double) => {
      ...
      CgnicodesLibrary.gni_lmm2(neq, func_sp, nstep, x, P, Q,xend, meth, null, iout, rpar, ipar)
