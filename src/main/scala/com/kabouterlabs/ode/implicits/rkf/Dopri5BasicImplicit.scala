...
import com.kabouterlabs.ode.rkf.Dopri5Basic

/**
  * Created by fons on 3/2/17.
  */
object Dopri5BasicImplicit {
  implicit val ev$FactoryT = new OdeSolverFactoryT[Double] {
    override type OdeSolverT = Dopri5Basic
    ...
    = HandleException {
      Some(new Dopri5Basic(dim, f, p, cf))
    }
  ...
  implicit class Ev$OdeRunT(solver: OdeM[Dopri5Basic]) extends OdeRunT[Dopri5Basic] {
  ...
  implicit object Ev$OdeSolverTC extends OdeSolverTC[OdeM[Dopri5Basic]]
  {
    ...
    override def createIvpEventSolver(dim: Int, f: OdeFuncM[Double], j: JacobianFuncM[Double], n: Int,
                                      c: ConstraintFuncM[Double], e: EventFuncM[Double], p: FuncParams[Double], cf: Config): OdeM[Dopri5Basic] = OdeM(dim,f,j,n,c,e,p,cf)
    ...
    override def createDaeSolver(dim: Int, f: OdeFuncM[Double], j: JacobianFuncM[Double], m: MassMatrixFuncM[Double], d: DaeIndexVariables,
                                 p: FuncParams[Double], cf: Config): OdeM[Dopri5Basic] = OdeM(dim,f,j,m,d,p,cf)

    override def solve(s: OdeM[Dopri5Basic], range: LineRangeT[Ev$OdeSolverTC.SolverDataType], init: Array[Ev$OdeSolverTC.SolverDataType]) = s(range, init)


    override def none = new OdeM[Dopri5Basic](None)
    ...
    override def createIvpSolver(dim: Int, f: OdeFuncM[Ev$OdeSolverTC.SolverDataType], j: JacobianFuncM[Ev$OdeSolverTC.SolverDataType],
                                 p: FuncParams[Ev$OdeSolverTC.SolverDataType], cf: Config): OdeM[Dopri5Basic] = OdeM(dim,f,j,p,cf)
  }
