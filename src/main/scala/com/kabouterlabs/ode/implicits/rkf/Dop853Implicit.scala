...
object Dop853Implicit
{
  implicit val ev$FactoryT = new OdeSolverFactoryT[Double] {
    ...
    =  HandleException{Some(new Dop853(dim,f, p, cf))}
  ...
  implicit class Ev$OdeRunT(solver:OdeM[Dop853]) extends OdeRunT[Dop853]
  {
  ...
  implicit object Ev$OdeSolverTC extends OdeSolverTC[OdeM[Dop853]]
  {
    ...
    override def createIvpEventSolver(dim: Int, f: OdeFuncM[Double], j: JacobianFuncM[Double], n: Int,
                                      c: ConstraintFuncM[Double], e: EventFuncM[Double], p: FuncParams[Double], cf: Config): OdeM[Dop853] = OdeM(dim,f,j,n,c,e,p,cf)
    ...
    override def createDaeSolver(dim: Int, f: OdeFuncM[Double], j: JacobianFuncM[Double], m: MassMatrixFuncM[Double], d: DaeIndexVariables,
                                 p: FuncParams[Double], cf: Config): OdeM[Dop853] = OdeM(dim,f,j,m,d,p,cf)

    override def solve(s: OdeM[Dop853], range: LineRangeT[Ev$OdeSolverTC.SolverDataType], init: Array[Ev$OdeSolverTC.SolverDataType]) = s(range, init)


    override def none = new OdeM[Dop853](None)
    ...
    override def createIvpSolver(dim: Int, f: OdeFuncM[Ev$OdeSolverTC.SolverDataType], j: JacobianFuncM[Ev$OdeSolverTC.SolverDataType],
                                 p: FuncParams[Ev$OdeSolverTC.SolverDataType], cf: Config): OdeM[Dop853] = OdeM(dim,f,j,p,cf)
  }
}
