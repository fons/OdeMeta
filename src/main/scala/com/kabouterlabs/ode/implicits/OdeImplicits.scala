...
object OdeImplicits {
  ...
  //implicit def convertFm2(f:OdeFuncInt):OdeFuncM[Double] = OdeFuncM[Double](Some({f}))

  implicit def convertIt(f:OdeFuncInt):OdeFuncUnit = (n:Int, x:Double, y:Array[Double], ydot:Array[Double]) => {f(n,x,y,ydot);Unit}
  ...
  implicit def convertConstrDouble(f:ConstraintFuncDoubleParam):ConstraintFuncM[Double] = ConstraintFuncM[Double](Some({f}))
  ...
  implicit def convertEventDouble(f:EventFuncDoubleParam):EventFuncM[Double] = EventFuncM[Double](Some({f}))
  ...
  implicit def convertMassMatrixFuncDouble(m:MassMatrixFuncDoubleParam):MassMatrixFuncM[Double] = MassMatrixFuncM[Double](Some({m}))
